...
public abstract class GeoGebraToPdf extends GeoGebraExport {
	...
	public void generateAllCode() {
		...
		// Initialize new StringBuilder for code
		// and CustomColor
		...
		if (format == GeoGebraToPdf.FORMAT_LATEX) {
			...
			for (it = 0; it < j + 2; it++) {
				if (it == 0) {
					...
					codeBeginDoc.append("::");
					codeBeginDoc.append(it);
					codeBeginDoc.append("\n");
				}
			}
			codeBeginDoc.append("\\end{filecontents}\n");
			codeBeginDoc.append("\\begin{document}\n");
			codeBeginDoc.append("\\begin{center}\n");
			codeBeginDoc.append("\\begin{animateinline}[\n");
			codeBeginDoc.append("begin={\n");
			codeBeginDoc.append(
			...
			// Clipping
			codeBeginDoc.append("\\clip");
			writePoint(xmin, ymin, codeBeginDoc);
			codeBeginDoc.append(" rectangle ");
		...
		/* Added from GeoGebraToPgf */
		// Close Environment tikzpicture
		if (format == GeoGebraToPdf.FORMAT_LATEX) {
			code.append("\\end{animateinline}\n");
	...
	protected void drawLocus(GeoLocus g) {
		...
		Iterator<MyPoint> it1 = ll.iterator();
		startBeamer(code);
		code.append("\\draw");
	...
	protected void drawBoxPlot(GeoNumeric geo) {
		...
		// Min-q1 horizontal
		codeFilledObject.append(" ");
		writePoint(min1, y, codeFilledObject);
		codeFilledObject.append("-- ");
	...
	protected void drawSumTrapezoidal(GeoNumeric geo) {
		...
		for (int i1 = 0; i1 < n1; i1++) {
			...
			writePoint(x[i1], 0, codeFilledObject);
			codeFilledObject.append(" -- ");
			writePoint(x[i1 + 1], 0, codeFilledObject);
			codeFilledObject.append(" -- ");
			writePoint(x[i1 + 1], y[i1 + 1], codeFilledObject);
			codeFilledObject.append(" -- ");
			writePoint(x[i1], y[i1], codeFilledObject);
	...
	protected void drawSumUpperLower(GeoNumeric geo) {
		...
		for (int i1 = 0; i1 < n1; i1++) {
			...
			writePoint(x[i1], 0, codeFilledObject);
			codeFilledObject.append(" rectangle ");
			writePoint(x[i1] + step1, y[i1], codeFilledObject);
			codeFilledObject.append(";\n");
			if (i1 != -1 && isBeamer) {
				codeFilledObject.append("  ");
			}
	...
	protected void drawAngle(GeoAngle geo) {
		...
		if (drawAngleAs(geo, EuclidianStyleConstants.RIGHT_ANGLE_STYLE_SQUARE)) {
			...
			for (int i1 = 0; i1 < 4; i1++) {
				writePoint(x[2 * i1], x[2 * i1 + 1], codeFilledObject);
				codeFilledObject.append(" -- ");
			}
			codeFilledObject.append("cycle; \n");
			endBeamer(codeFilledObject);
	...
	protected void drawSlider(GeoNumeric geo) {
		...
		double max1 = geo.getIntervalMax();
		double min1 = geo.getIntervalMin();
		...
		double param = (value - min1) / (max1 - min1);
	...
	protected void drawPolygon(GeoPolygon geo) {
		...
		for (int i1 = 0; i1 < points.length; i1++) {
			Coords coords = points[i1].getCoordsInD2();
	...
	protected void drawPolyLine(GeoPolyLine geo) {
		...
		for (int i1 = 0; i1 < path.length; i1++) {
			Coords coords = path[i1].getInhomCoords();
	...
	protected void drawHistogramOrBarChartBox(double[] y, double[] x,
		...
		if (command.contains("Binomial") && command.contains("true")) {
			...
			for (int i1 = 0; i1 < length - 1; i1++) {
				...
				writePoint(x[i1] + width / 2, y[i1], codeFilledObject);
				codeFilledObject.append(" -- ");
				writePoint(x[i1 + 1] + width / 2, y[i1], codeFilledObject);
				codeFilledObject.append(";\n");
				codeFilledObject.append("\\draw");
				s = lineOptionCode(g, true);
				if (s.length() != 0) {
				...
				writePoint(x[i1 + 1] + width / 2, y[i1], codeFilledObject);
				codeFilledObject.append(" -- ");
				writePoint(x[i1 + 1] + width / 2, y[i1 + 1], codeFilledObject);
				codeFilledObject.append(";\n");
			}
		} else {
			for (int i1 = 0; i1 < length; i1++) {
				...
				codeFilledObject.append(" rectangle ");
				if (x.length == length) {
					writePoint(x[i1] + width, y[i1], codeFilledObject);
				} else {
					writePoint(x[i1 + 1], y[i1], codeFilledObject);
				}
				codeFilledObject.append(";\n");
			}
		}
	...
	protected boolean fillSpline(GeoCurveCartesian[] curves) {
		...
		for (int i1 = 0; i1 < curves.length; i1++) {
			p = curves[i1].getMinParameter();
			y = curves[i1].getFunY().value(curves[i1].getMinParameter());
			yprec = y;
			if (Math.abs(y) < 0.001) {
				y = yprec = 0;
			}
			double step1 = (curves[i1].getMaxParameter()
					- curves[i1].getMinParameter()) / 200;
			xprec = curves[i1].getFunX().value(curves[i1].getMinParameter());
			x = xprec;
			for (; p <= curves[i1].getMaxParameter(); p += step1) {
				y = curves[i1].getFunY().value(p);
				x = curves[i1].getFunX().value(p);
				if (Math.abs(y) < 0.001) {
					y = 0;
				}
				if (Math.abs(x) < 0.001) {
